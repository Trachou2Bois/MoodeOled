...
- **(Pas testé) Support GPIO et encodeur rotatif** utilisant `lgpio` (compatible Raspberry Pi 4 et 5 sous Bookworm). Nécessite d'être activé et de configurer les broches dans le config.ini dans la section "manual".
...
- **Système d'exploitation** : Raspberry Pi OS Bookworm (Moode ≥ 9.3.7 requis).

- **Matériel** : Raspberry Pi (Zero 2W, 3, 4, 5 recommandé).
